...
# 用于指定在接收到信号后将要采取的动作。
# arg 接收到信号时执行的指令 signal_spec 信号名称或信号名称对应的数字
...
function generate_piece() {
  while true; do
    ...
    # 判断board[$pos]位置是否为0,不为0继续随机生成要产生数字的位置,否则进入语句中,为该位置产生值
    let board[$pos] || {
...
#
# 如果是up,就按行从上往下遍历,因为方块都要往上走,
# 如果是down,就按行从下往上遍历,因为方块都是要往下走
# 如果是left 就按列从左往右算,因为所有方块都往左走
...
function push_pieces {
  ...
  let ${board[$first]} || {
    # second位置上的数不为0 往下执行
    let ${board[$second]} && {
      if test -z $5; then # test -z 如果 $5参数 为空 z - zero,则为真
        board[$first]=${board[$second]}
        let board[$second]=0
        let change=1
        printf "move piece with value ${board[$first]} from [$second] to [$first]\n" >&3
      else #如果是检查能不能移动，只需要moves++,否则就需要进行方块的移动
        let moves++
      fi
      return
    }
  ...
  # second位置上的数不为0时,将flag_skip 置为1
  ...
  for i in $(_seq 0 $index_max); do
    for j in $(_seq 0 $index_max); do
      ...
      for k in $(_seq 1 $increment_max); do
        # 如果 flag_skip 为真 则break
        let flag_skip && break
        # 进行滑块的移动 $2 这个参数只有在 check_moves方法里会传入
...
#btlrh都是参数选项,选项后面有: 表示该选项必须有一个参数
while getopts "b:t:l:rh" opt; do
  case $opt in
  # OPTARG是全局变量 表示某个参数选项的参数
    b ) board_size="$OPTARG"
    ...
    t ) target="$OPTARG"
      #obase=2 表示设置2进制,传入target数值,经过bc转换成2进制,用grep正则过滤
...
let pieces=0 #记录当前有多少位置有数字
generate_piece  #随机在一个位置生成数字
first_round=$last_added
generate_piece #再生成一个数字
